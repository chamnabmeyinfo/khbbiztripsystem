...
| `PackageEditorModal` | `PackageEditorModal.tsx` | Detailed modal editor for creating/updating multi-day tour packages with Dual-mode Navigation (Aside Menu Style with Search and Tab Style with horizontal scrolling pills), Inner Sub-Section Controller, 7 modular studio sections, complete **Tab 4 Day-by-Day Mission Schedule** & bilingual **Hour-by-Hour Guide & Escort Timeline** slot builder with 1-Click AI auto-translation, explicit color-coded **Language Indicators** (`🇬🇧 English (EN)` blue badge & `🇰🇭 ខ្មែរ (KM)` amber badge), background **Auto-Save Engine** (saves form drafts to local storage with timestamp badges `💾 Auto-Saved: HH:mm:ss`), high-visibility **Save Success Toast Notification** (`✅ Tour Package Updated & Synchronized!`), smart missing translation detection with 1-Click Auto-Fill (`⚡ 1-Click Auto-Fill English/Khmer`), zero TDZ initialization ordering, safe asynchronous promise resolution & unmount lifecycle guards, real-time **👁️ Live Preview Modal** overlay (`constructPreviewPackage()`, bilingual preview toggle, and desktop/mobile viewport switcher), AI language copilot, and persistent sticky action footer (`sticky bottom-0`) |
...
| `FieldAiTranslator` | `FieldAiTranslator.tsx` | Reusable inline AI translation button (single/dual text & array modes) with script-validated results, a per-button **Translation Model Picker dropdown** (Auto + Gemini/OpenAI/DeepSeek/Groq models, persisted in `localStorage`), and model-used success indicators |
