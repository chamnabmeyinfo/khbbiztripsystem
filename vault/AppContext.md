...
// - Safe LocalStorage Quota Guard (`src/utils/safeStorage.ts`): Eliminates browser `QuotaExceededError` crashes by wrapping all localStorage writes in `safeSetItem()`. Heavy deleted items are automatically sanitized via `sanitizeDeletedItemsForStorage()` (stripping base64 blobs/images while preserving full record snapshots in Cloud Firestore). On quota exhaustion, `cleanStorageForQuotaRelief()` executes emergency cleanup of expendable draft keys and bloated logs. On startup, `initStorageSanitizer()` checks and cleans bloated local storage keys before React state initialization. `ErrorBoundary` includes automatic quota detection and an emergency storage relief recovery action.
